...
export default function TrainerNotifications() {
  ...
  const handleSend = async (e) => {
    e.preventDefault();
    if (!targetType) return showToast('Please select a target', 'error');
    if (needsBatch && !selectedBatches.length) return showToast('Please select at least one batch', 'error');
    if (needsUsers && !selectedUsers.length) return showToast('Please select at least one student', 'error');

    setSending(true);
    try {
      const res = await axios.post('/notifications/trainer/bulk', {
        title,
        message,
  ...
  return (
    <div className="min-h-full bg-gray-50 p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1">Trainer · Student Communication</p>
      </div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="lg:col-span-3 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <h2 className="font-bold text-gray-800 text-lg mb-5 flex items-center gap-2">
              <Send className="w-4 h-4 text-blue-600" /> Compose Message
            </h2>

            <form onSubmit={handleSend} className="space-y-5">
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">Target Students</label>
              ...
              <div>
                ...
                <input type="text" required value={title} onChange={e => setTitle(e.target.value)}
                  placeholder="e.g., Tomorrow's Class Link"
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {!needsBatch && !needsUsers && (
              <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white flex flex-col justify-center items-center text-center gap-4 min-h-[240px]">
                ...
                <div>
                  <p className="font-bold text-lg">Broadcast to Students</p>
                  <p className="text-blue-200 text-sm mt-1">
                    {targetType ? 'This will send to all students in your assigned batches.' : 'Select a target on the left to get started.'}
                  </p>
                </div>
              </div>
            )}
